function planlight, map1, target, inst, dust=dust
  ...
  if keyword_set(dust) then begin
    zodipic, fnu, 1000*inst.platescale, inst.lambda, pixnum = inst.pixnum, $
      inclination = target.pinc, radin = target.sdist*inst.iwa*1.1, radout = target.sdist*inst.owa*1.1, albedo = target.palb_geo, zodis = 1, $
      starname = target.pname, rstar = target.srad, lstar = 10.^target.slum, tstar = target.stmod, distance = target.sdist, $
