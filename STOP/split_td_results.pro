pro split_td_results, file, split = split, celsius = celsius
  ...
  ; Open file
  counter = 1
  openr, 1, sett.tdpath + 'export/' + file + '.dat'

  if keyword_set(split) then $
    openw, 2, sett.tdpath + 'export/' + file + '_' + n2s(counter) + '.dat' else $
    openw, 2, sett.tdpath + 'export/' + file + '_single.dat'
  ...
  while not eof(1) do begin
    ...
    if (strmatch(line, '*TIME*')) then begin
      if keyword_set(split) then begin
        ...
        counter++
        openw, 2, sett.tdpath + 'export/' + file + '_' + n2s(counter) + '.dat'
